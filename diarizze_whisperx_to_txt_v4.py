...
- merge_short_turns(): merges very short turns (< 0.3s) that are likely
  mis-assigned noise, without over-correcting real alternating speech.
...
python src/diarizze_whisperx_to_txt_v4.py --audio data/raw_audio/Episode09-clip.wav --outdir outputs --model medium --compute_type int8 --language en   
...
def merge_short_turns(turns, min_turn_duration=0.3):
    ...
    Short turns (< min_turn_duration) during fast back-and-forth
    are often mis-assigned noise. Merging them improves accuracy
    without over-correcting real alternating speech.

    e.g:
    Speaker 1: 0.0 - 5.0
    Speaker 2: 5.1 - 5.2  <- too short, likely noise
    Speaker 2: 5.3 - 10.0
    ->
    Speaker 1: 0.0 - 5.2  <- merged
    Speaker 2: 5.3 - 10.0
    """
    if not turns:
        return turns
...
def main():
    ...
    ap.add_argument("--min_turn_duration", type=float, default=0.3,
                    help="Turns shorter than this (sec) are merged into previous turn (default: 0.3)")
    args = ap.parse_args()
    ...
    # Merge short turns to fix rapid turn-taking mis-assignments
