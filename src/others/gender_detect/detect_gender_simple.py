#!/usr/bin/env python3
"""
Detect speaker gender using voice analysis for simple diarization output.
...
Usage:
  python detect_gender_simple.py \
    --audio data/raw_audio/ep_001.wav \
    --input-json outputs/ep_001.diarized.json \
    --output-json outputs/ep_001.diarized.gender.json \
    --output-txt outputs/ep_001.diarized.gender.txt
"""
...
def update_txt_with_gender(txt_path, json_path, speaker_gender, output_path):
    ...
    # Create mapping from speaker label to gender
    speaker_gender_map = {}
    for speaker_info in json_data.get('speakers', []):
        speaker_id = speaker_info.get('speaker_id', 0)
        speaker_label = speaker_info.get('speaker_label', f"Speaker {speaker_id + 1}")
        gender = speaker_gender.get(speaker_id, 'unknown')
        speaker_gender_map[speaker_label] = gender
    ...
    for line in lines:
        ...
        if line and '[' in line and ']' in line and ':' in line:
            # Parse line format: [00:00:00] Speaker 1: text
            parts = line.split(':', 1)
            if len(parts) >= 2:
                speaker_part = parts[0].strip()  # [00:00:00] Speaker 1
                text_part = parts[1].strip()    # text
                
                # Extract speaker label from speaker_part
                if ']' in speaker_part:
                    timestamp = speaker_part.split(']')[0] + ']'
                    speaker_label = speaker_part.split(']')[1].strip()
                    gender = speaker_gender_map.get(speaker_label, 'unknown')
                    
                    # Add gender info
                    updated_line = f"{timestamp} {speaker_label} ({gender}): {text_part}"
    ...
    for speaker_label, gender in speaker_gender_map.items():
        gender_summary += f"# {speaker_label}: {gender}\n"
...
def main():
    ap = argparse.ArgumentParser(description="Detect speaker gender from audio and update simple diarization files")
    ap.add_argument("--audio", required=True, help="Audio file path")
    ap.add_argument("--input-json", required=True, help="Input diarized JSON file path")
    ...
    print(f"Loading diarized transcript: {input_json_path}")
    ...
    input_txt_path = input_json_path.parent / f"{input_json_path.stem}.diarized.txt"
    if input_txt_path.exists():
        ...
        # Create TXT from JSON if not found
        print("Creating TXT from JSON...")
        segments_with_gender = []
        for seg in segments:
            speaker_id = seg.get('speaker_id', 0)
            gender = speaker_gender.get(speaker_id, 'unknown')
            ...
            if text:
                segments_with_gender.append(f"[{timestamp}] {speaker_label} ({gender}): {text}")
        
        with open(output_txt_path, 'w', encoding='utf-8') as f:
            f.write('\n'.join(segments_with_gender))
        
        print(f"Created TXT: {output_txt_path}")
