...
def detect_gender_mock(segments):
    """
    Mock gender detection for testing purposes.
    Returns dict: {speaker_id: gender}
    """
    # Simple heuristic: speaker 0 is male (host), speaker 1 is female (guest)
    speaker_gender = {}
    ...
    for speaker_id in speakers:
        if speaker_id == 0:
            speaker_gender[speaker_id] = 'male'
        elif speaker_id == 1:
            speaker_gender[speaker_id] = 'female'
        else:
...
def label_transcript_with_gender(segments, speaker_gender):
    """
    Label transcript with host/guest roles and add gender information.
    """
    # Determine host by speaking time (heuristic)
    ...
    host_id = max(speaker_times.items(), key=lambda x: x[1])[0] if speaker_times else 0
...
def process_single_file(input_file, output_dir, method):
    ...
    try:
        ...
        # Gender detection
        if method == "mock":
            speaker_gender = detect_gender_mock(segments)
        else:
        ...
        json_output = output_dir / f"{base_name}.host_guest.json"
        txt_output = output_dir / f"{base_name}.host_guest.txt"
