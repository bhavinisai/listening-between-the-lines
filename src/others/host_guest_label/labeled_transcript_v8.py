...
TXT output now preserves richer segment metadata, e.g.:
...
Usage:
  ...
  python src/labeled_transcript_v8.py --input data/outputs/whisperx/ep_002_whisperx_diarized.gender.json --out_dir data/outputs/whisperx/ --model anthropic/claude-3-haiku     --debug_dump_raw
...
def build_txt_speaker_label(seg: Dict[str, Any], speaker_key: str) -> str:
    """
    Preserve speaker_raw and append any available metadata in parentheses.
    Example:
      SPEAKER_00 (male, HOST)
      SPEAKER_01 (GUEST)
    """
...
def main():
    ...
    base = in_path.stem
    debug_primary = others/ f"{base}.openrouter_primary.response.json" if args.debug_dump_raw else None
    debug_repair = others / f"{base}.openrouter_repair.response.json" if args.debug_dump_raw else None
    debug_text_primary = others / f"{base}.openrouter_primary.content.txt" if args.debug_dump_raw else None
    debug_text_repair = others / f"{base}.openrouter_repair.content.txt" if args.debug_dump_raw else None
