#!/usr/bin/env python3
"""
labeled_transcript_v10.py

- Loads WhisperX transcript JSON (expects `segments[]` with `speaker` or `speaker_raw` + `text`)
- Uses OpenAI API to pick HOST speaker (exactly one) and labels others as GUEST
- Writes:
...
Robustness:
- Captures full OpenAI response JSON when --debug_dump_raw is set
...
Usage:
  ...
  python src/labeled_transcript_v10.py \
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            if resp.status_code == 429:
                ...
                print(f"INFO: rate limited (429), waiting {wait:.1f}s before retry {attempt}/{retries}...", file=sys.stderr)
                time.sleep(wait)
                continue
            ...
            if attempt < retries:
                ...
                continue
            raise RuntimeError(
                f"OpenAI request failed after {retries} attempts: {last_err}"
            ) from last_err
...
def repair_to_json(
    ...
    repair_messages = [
        ...
        {"role": "user", "content": f"""Convert the following into VALID JSON for this schema:
...
# -----------------------------
def main():
    ap = argparse.ArgumentParser(description="Label HOST/GUEST in WhisperX transcripts using OpenAI.")
    ...
    ap.add_argument("--model", default="gpt-4o-mini", help="OpenAI model name (e.g. gpt-4o-mini, gpt-4o)")
    ...
    ap.add_argument("--retries", type=int, default=4, help="Number of API call retries")
    ap.add_argument("--add_role_to_words", action="store_true")
    ap.add_argument("--debug_dump_raw", action="store_true", help="Save raw OpenAI responses to others/")
    args = ap.parse_args()
    ...
    base = in_path.stem
    debug_primary      = others / f"{base}.openai_primary.response.json" if args.debug_dump_raw else None
    debug_repair       = others / f"{base}.openai_repair.response.json"  if args.debug_dump_raw else None
    debug_text_primary = others / f"{base}.openai_primary.content.txt"   if args.debug_dump_raw else None
    debug_text_repair  = others / f"{base}.openai_repair.content.txt"    if args.debug_dump_raw else None
    ...
    try:
        ...
        print(f"WARNING: primary OpenAI call failed: {e}", file=sys.stderr)
        try:
