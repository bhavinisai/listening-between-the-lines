#!/usr/bin/env python3
"""
labeled_transcript_v9.py

- Loads WhisperX transcript JSON (expects `segments[]` with `speaker` or `speaker_raw` + `text`)
- Uses Google Gemini API to pick HOST speaker (exactly one) and labels others as GUEST
- Writes:
...
Robustness:
- Captures full Gemini response JSON when --debug_dump_raw is set
...
Usage:
  ...
  python src/labeled_transcript_v9.py \
...
def build_prompt_text(samples: Dict[str, List[Tuple[float, float, str]]], filename_hint: str) -> str:
    """Build the full prompt as a single string for Gemini."""
    ...
    return f"""You are labeling speakers in a podcast transcript.
...
def gemini_generate(
    ...
    Returns (text_content, full_response_json).
    Uses JSON mime type to encourage structured output.
    """
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            content = ""
            try:
                candidates = last_json.get("candidates", [])
                if candidates:
            ...
            if attempt < retries:
                ...
                continue
            raise RuntimeError(
                f"Gemini request failed after {retries} attempts: {last_err}"
            ) from last_err
...
def repair_to_json(
    ...
    """Ask Gemini to fix a malformed JSON response."""
    repair_prompt = f"""Convert the following into VALID JSON for this schema:
...
# -----------------------------
def main():
    ap = argparse.ArgumentParser(description="Label HOST/GUEST in WhisperX transcripts using Gemini.")
    ...
    ap.add_argument("--debug_dump_raw", action="store_true", help="Save raw Gemini responses to others/")
    args = ap.parse_args()
    ...
    base = in_path.stem
    debug_primary      = others / f"{base}.gemini_primary.response.json"  if args.debug_dump_raw else None
    debug_repair       = others / f"{base}.gemini_repair.response.json"   if args.debug_dump_raw else None
    debug_text_primary = others / f"{base}.gemini_primary.content.txt"    if args.debug_dump_raw else None
    debug_text_repair  = others / f"{base}.gemini_repair.content.txt"     if args.debug_dump_raw else None

    host_id: Optional[str] = None
    result: Dict[str, Any] = {}

    try:
        prompt = build_prompt_text(samples, filename_hint=in_path.name)
        ...
        print(f"WARNING: primary Gemini call failed: {e}", file=sys.stderr)
        try:
            ...
            print(f"WARNING: repair call also failed: {e2}. Falling back to heuristic.", file=sys.stderr)
            result = {}
    ...
    if isinstance(result, dict):
        ...
        if candidate and str(candidate) in all_speakers:
            ...
            if candidate:
                print(f"WARNING: model returned unknown speaker '{candidate}', using heuristic.", file=sys.stderr)
