...
One script to:
1) Batch transcribe multiple audio files with Deepgram (REST /v1/listen)
2) Save per-file JSON + plain transcript
3) Convert response into diarized TXT lines like:
   [HH:MM:SS.mmm - HH:MM:SS.mmm] Speaker N: text
4) Save simple speaker stats (talk-time + turns)

Usage:
  export DEEPGRAM_API_KEY="YOUR_KEY"

  # Folder mode
  python src/deepgram_transcribe_and_format.py \
    --input-dir data/raw_audio \
    --out-dir outputs \
    --diarize --utterances --smart-format

  # Glob mode
  python src/deepgram_transcribe_and_format.py \
    --files data/raw_audio/*.* \
    --out-dir outputs \
    --diarize --utterances
"""
...
def transcribe_and_format_one(
    ...
    if not overwrite and json_out.exists():
        # If you want: still regenerate diarized txt from existing JSON.
...
def parse_args() -> argparse.Namespace:
    ...
    # diarized text segmentation fallback
    p.add_argument("--gap-s", type=float, default=0.8, help="Gap threshold for word-based segmentation.")
    return p.parse_args()
